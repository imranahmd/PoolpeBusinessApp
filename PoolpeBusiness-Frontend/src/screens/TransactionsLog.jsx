...
const TransactionsLog = ({route, navigation, resetUnseenCount}) => {
  ...
  const [activeScreen, setActiveScreen] = useState(''); // New state to track the active screen
  const [isFilterClicked, setIsFilterClicked] = useState(false); // State to track if filter is clicked without dates
  ...
  const fetchData = async () => {
    try {
      ...
      if (data && data.data && data.data.fundOrderList) {
        ...
        setTransactions(data.data.fundOrderList);
        // setFilteredTransactions(data.data.fundOrderList);
      }
      if (data && data.data && data.data.fundOrderList.length > 0) {
        // Assuming the transactions are sorted with the most recent first
        const mostRecentTransactionTime = data.data.fundOrderList[0].txnTime;

        await AsyncStorage.setItem(
          '@last_seen_transaction_time',
          new Date(mostRecentTransactionTime).toString(),
        );
        console.log(
          'Updated @last_seen_transaction_time:',
          new Date(mostRecentTransactionTime).toString(),
        );
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      //       `${apiUrl}api/fundorder/createfundorder`,
      //       {
      //         method: 'POST',
      //         headers: {
      //           'Content-Type': 'application/json',
      ...
      if (transactions.length > 0) {
        AsyncStorage.setItem(
          '@last_seen_transaction_time',
          new Date(transactions[0].txnTime).toString(),
        );
      }
    }, []),
  );
  ...
  useEffect(() => {
    ...
    const fetchQrCode = () => {
      fetch(`${apiUrl}api/qr/createqr`, {
        ...
        .then(data => {
          if (data.status === 201) {
            const qrImageStr = data.data.qrString; // Adjusted based on response structure
            setQrImage(
  ...
  useFocusEffect(
    useCallback(() => {
      // resetUnseenCount;
      // Reload transactions when the screen is focused again
      fetchData();
    }, []),
  );
  ...
  const filterTransactions = () => {
    ...
    if (!startDate || !endDate) {
      setFilteredTransactions([]); // Clear filtered transactions if dates are not entered
      return;
    }

    const filtered = transactions.filter(transaction => {
      const txnDate = new Date(transaction.txnTime);
    ...
    if (screen === 'Home') {
      try {
        ...
        navigation.navigate('Dashboard', {
          ...
          // resetUnseenCount: () => {
          //   AsyncStorage.setItem('@unseenTransactionsCount', '0'); // Persist reset in AsyncStorage
          // },
  ...
  const renderTransactionItem = ({item}) => {
    return <TransactionItem transaction={item} styles={styles} />;
  };
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <>
          ...
          {isFilterClicked && filteredTransactions.length === 0 ? (
...
const TransactionItem = ({transaction, styles}) => {
    ...
    <View
      ...
      <TouchableOpacity onPress={() => setExpanded(!expanded)}>
        {/* <View style={styles.transactionRow}>
          ...
          <Text style={styles.transactionType}>{transaction.payerVPA}</Text>
          <Text style={styles.transactionType}>{transaction.txnTime}</Text>
        </View> */}
        <View style={styles.transactionRow}>
          <View style={styles.leftColumn}>
            ...
            <Text style={styles.transactionTime}>{transaction.txnTime}</Text>
          </View>
        ...
        {expanded && (
          <View style={styles.transactionDetails}>
            <Text style={styles.bold}>
              Transaction ID:{' '}
              <Text style={styles.tnID}>{transaction.txnId}</Text>
            </Text>
            <Text style={styles.bold}>
              Message:{' '}
              <Text style={styles.tnID}>{transaction.responseMessage}</Text>
            </Text>
            <Text style={styles.bold}>
              UTR: <Text style={styles.tnID}>{transaction.utr}</Text>
            </Text>
          </View>
        )}
      </TouchableOpacity>
